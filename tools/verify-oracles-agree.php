 ...
 * exits non-zero.
 *
 * When comparing Jalali fixtures generated by different ICU versions, rows
 * whose JDN falls within JalaliIcuDivergence::RANGES are expected to differ
 * (Birashk-vs-ICU disagreement window) and are reported but not treated as
 * failures.
 *
 * Usage:
...
foreach ($pairs as [$phpFile, $nodeFile]) {
    ...
    while (true) {
        ...
        if ($aStr !== $bStr) {
            if ($tolerateJalali) {
                $row = json_decode($aStr, true);
                $jdn = $row['jdn'] ?? null;
                if ($jdn !== null && JalaliIcuDivergence::contains($jdn)) {
                    $skippedDivergence++;
                    continue;
                }
            }
    ...
    if ($skippedDivergence > 0) {
        fprintf(STDOUT, "  ⊘ %d rows skipped (known Jalali ICU divergence)\n", $skippedDivergence);
    }
    if ($diffs === 0) {
